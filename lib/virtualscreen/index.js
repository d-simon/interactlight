...
Screen.prototype.update = function (array) {
    for (var i = 0; i < array.length; i++) {
        for (var j = 0; j < array[i].length; j++) {
            this.screen[array[i]][array[j]] = new this.Pixel(array[i][j][0], array[i][j][1], array[i][j][2]);
        }
    }
};
Screen.prototype.updateObj = function (obj) {
    for (var px in obj) {
        this.screen[obj[px].y][obj[px].x] = new this.Pixel(obj[px].r, obj[px].g, obj[px].b);
    }
};
