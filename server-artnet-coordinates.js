...
var reduceInterval =  setInterval(function () {
    ...
    for (var i = 0; i < screenHeight; i++) {
        for (var j = 0; j < screenWidth/2; j++) {
            ...
            obj[(i*screenWidth/2+j)*3+2] = (dmx.universes[0].get((i*screenWidth/2+j)*3+2) > 0) ? reduceUntil(dmx.universes[0].get((i*screenWidth/2+j)*3+2),1) : reduceUntilZero(dmx.universes[0].get((i*screenWidth/2+j)*3+2),10);
        }
    }
    for (var i = 0; i < screenHeight; i++) {
        for (var j = 0; j < screenWidth/2; j++) {
            ...
            obj2[(i*screenWidth/2+j)*3+2] = (dmx2.universes[0].get((i*screenWidth/2+j)*3+2) > 0) ? reduceUntil(dmx2.universes[0].get((i*screenWidth/2+j)*3+2),1) : reduceUntilZero(dmx2.universes[0].get((i*screenWidth/2+j)*3+2),10);
        }
    }
    // for (var i = 0; i < screenHeight; i++) {
    //     for (var j = 0; j < screenWidth/2; j++) {
    //         obj[(i*screenWidth/2+j)*3+0] = reduceUntilZero(dmx.universes[0].get((i*screenWidth/2+j)*3+0),5);
    //         obj[(i*screenWidth/2+j)*3+1] = reduceUntilZero(dmx.universes[0].get((i*screenWidth/2+j)*3+1),5);
    //         obj[(i*screenWidth/2+j)*3+2] = reduceUntilZero(dmx.universes[0].get((i*screenWidth/2+j)*3+2),10);
    //     }
    // }
    // for (var i = 0; i < screenHeight; i++) {
    //     for (var j = 0; j < screenWidth/2; j++) {
    //         obj2[(i*screenWidth/2+j)*3+0] = reduceUntilZero(dmx2.universes[0].get((i*screenWidth/2+j)*3+0),5);
    //         obj2[(i*screenWidth/2+j)*3+1] = reduceUntilZero(dmx2.universes[0].get((i*screenWidth/2+j)*3+1),5);
    //         obj2[(i*screenWidth/2+j)*3+2] = reduceUntilZero(dmx2.universes[0].get((i*screenWidth/2+j)*3+2),10);
    //     }
    // }
    dmx.update(0, obj);
    dmx2.update(0, obj2);
    ...
    stream.on('data', function(data) {
        ...
        if (data.geo) {
            ...
            if (array[0].x < 12) {
                ...
                obj[led*3+2] = Math.min(dmx.universes[0].get(led*3+2) + 10, 50);
                ...
                obj2[led*3+2] = Math.min(dmx2.universes[0].get(led*3+2) + 10, 50);
            }
        }
